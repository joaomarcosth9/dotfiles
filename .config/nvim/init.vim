...
" copy templates
autocmd filetype cpp nnoremap <F1> :r $HOME/Templates/cpp_singletest.cpp <Return>kdd :12 <Return> o
autocmd filetype cpp nnoremap <F2> :r $HOME/Templates/cpp_multitest.cpp <Return> kdd :12 <Return> o
...
autocmd filetype c nnoremap <F1> :r $HOME/Templates/cdefault.c <Return>kdd :4 <Return> o

" Filetypes autocmds to compile and run
autocmd filetype python nnoremap <F4> :w <bar> term s -std 17 '%' <CR>
autocmd filetype c nnoremap <F4> :w <bar> term s '%' <CR>
autocmd filetype haskell nnoremap <F4> :w <bar> term s '%' <CR>
...
autocmd filetype cpp nnoremap <F8> :w <bar> term c++ --std=c++20 '%' -O2 -fsanitize=address,undefined -o /tmp/out && /tmp/out <CR>
...
command! -nargs=0 TplPicker lua require('telescope.builtin').find_files({ prompt_title = 'TPL Files', cwd = '~/tpl', hidden = true })<CR>
